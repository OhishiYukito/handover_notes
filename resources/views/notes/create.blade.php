...
<x-app-layout>
    ...
    <html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
        ...
        <body>
            ...
            <h1 class="text-3xl text-center m-8">新しいノートの作成</h1>
            <div class="relative">
                <!-- input format -->
                <form action="/notes" method="POST">
                    ...
                    <div class="flex justify-evenly">
                        ...
                        <p>年度:&emsp;
                            <input type="text" list="event_year" name="event[event_year]" placeholder="入力または選択" autocomplete="off" value="{{ old('event.event_year') }}">
                        ...
                        <p>イベント名:&emsp;
                            <input type="text" list="event_name" name="event[event_name]" placeholder="入力または選択" autocomplete="off" value="{{ old('event.event_name') }}">
                        ...
                        <p>タグ:&emsp;
                            <input type="text" list="event_tag" name="event[tag]" placeholder="入力または選択" autocomplete="off" value="{{ old('event.tag') }}">
                    ...
                    <input class="ml-8 mt-8" type="text" name="event[title]" size=100 placeholder="タイトルを入力" autocomplete="off" value="{{ old('event.title') }}"></input>
                    <p class="ml-8 mb-8" style="color:red">{{ $errors->first('event.title') }}</p>
                    
                    <textarea class="ml-8 mr-8 mb-4" cols=100 rows=15 name="event[text]" placeholder="本文を入力" value="{{ old('event.text') }}"></textarea>
                    <p class="ml-8" style="color:red">{{ $errors->first('event.text') }}</p>
                    
                    <input class="absolute right-10 mt-4" type="submit" value="作成"></input>
